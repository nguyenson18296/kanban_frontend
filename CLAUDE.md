...
- **Board rooms (JAV-32):** the open board subscribes to live updates via `board:join`/`board:leave` (`SocketManager.joinBoardRoom`/`leaveBoardRoom` + the colocated `useBoardRoom(projectId)` hook in KanbanBoard). The join is re-sent on every `connection:established` — a reconnect is a new session and forgets rooms. `board:join:error` is deliberately generic (masks "no access" vs "doesn't exist"): the board renders the same "Project not found" state as an HTTP 404 and never auto-retries (the next attempt is the next connect); a denied join leaves the socket — and notifications/presence — untouched. On `connection:error` the manager now disconnects, refreshes the token once, and reconnects; a second consecutive auth failure stays disconnected (no reconnect loop).
